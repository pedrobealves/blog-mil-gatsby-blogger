...
exports.sourceNodes = async ({
  ...
  if (posts) {
    posts.forEach(post => {
      unified().use(parse).use(rehype2remark).use(stringify).process(post.content, function (err, md) {
...
${md}`,
  ...
  if (pages) {
    pages.forEach(page => {
      unified().use(parse).use(rehype2remark).use(stringify).process(page.content, function (err, md) {
        if (err) console.log(err);
